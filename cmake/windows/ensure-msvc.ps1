# Bootstrap MSVC for Scyclone on Windows (one-time, or after a VS update).
# Installs/pins toolset 14.51 if needed and writes cmake/windows/generated/msvc-env.cmake.
# After this, use plain cmake --preset / cmake --build like on Linux and macOS.
#
# Usage:
#   .\cmake\windows\ensure-msvc.ps1
#   .\cmake\windows\ensure-msvc.ps1 -ToolsetVersion 14.44
#   .\cmake\windows\ensure-msvc.ps1 -AutoInstallToolset   # install missing toolset without prompting
#   .\cmake\windows\ensure-msvc.ps1 -NoToolsetInstall     # fail if toolset missing (CI-friendly)
...
function Assert-VsInstallerSucceeded {
    ...
    if ($ExitCode -eq 3010) {
        Write-Warning "$OperationDescription succeeded; reboot Windows when convenient, then rerun ensure-msvc if the toolset is still missing."
        return
    }
...
function Write-MsvcEnvCMake {
    ...
    $lines = @(
        '# Generated by cmake/windows/ensure-msvc.ps1 - do not edit',
        "# Toolset: $ToolsetName",
...
Write-Host "Using MSVC toolset $toolsetInUse for Scyclone."
...
Write-Host 'Next: cmake --preset release   (or default / asan)'
