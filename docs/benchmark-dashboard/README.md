...
Source layout for the public benchmark charts. CI updates **`data.js` only** on the `gh-pages` branch; this `index.html` is **not** overwritten once present.

## Preview locally

```powershell
cd docs/benchmark-dashboard
...
Copy-Item docs/benchmark-dashboard/index.html dev/bench/
Copy-Item docs/benchmark-dashboard/torsion-audio-lockup.svg dev/bench/
Copy-Item docs/benchmark-dashboard/favicon.svg dev/bench/
Copy-Item docs/benchmark-dashboard/fonts/*.ttf dev/bench/fonts/
...
1. Edit `docs/benchmark-dashboard/index.html` in this repo (CSS is in the `<style>` block).
